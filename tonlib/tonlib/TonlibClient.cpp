...
bool TonlibClient::is_static_request(td::int32 id) {
  switch (id) {
    ...
    case tonlib_api::ftabi_generateExtendedStateInit::ID:
 ...
 private:
  ...
  td::Status do_start_up() {
    ...
    if (has_private_key_) {
      ...
      is_fake_key_ = input_key.first == KeyStorage::InputKeyType::Fake;
      public_key_ = td::Ed25519::PublicKey(input_key.second.key.public_key.copy());
...
void create_message_body(KeyStorage& key_storage, const tonlib_api::ftabi_createMessageBody& request,
                         td::Promise<tonlib_api_ptr<tonlib_api::ftabi_messageBody>>&& promise) {
  TRY_RESULT_PROMISE(promise, function, parse_function(*request.function_))
  TRY_RESULT_PROMISE(promise, function_call, parse_function_call(key_storage, *function, request.call_))
  TRY_RESULT_PROMISE(promise, body, function->encode_input(function_call))
  TRY_RESULT_PROMISE(promise, serialized, vm::std_boc_serialize(body))
...
td::Status TonlibClient::do_request(tonlib_api::ftabi_runLocal& request,
  ...
  TRY_RESULT(function_call, parse_function_call(key_storage_, *function, request.call_))
...
auto unpack_from_cell(const tonlib_api::ftabi_unpackFromCell& request)
    -> td::Result<tonlib_api_ptr<tonlib_api::ftabi_decodedOutput>> {
  TRY_RESULT(cell, from_bytes(request.cell_->bytes_))
  TRY_RESULT(params, parse_params(request.params_))
  TRY_RESULT(values, ftabi::unpack_from_cell(cell, params))
  return tonlib_api::make_object<tonlib_api::ftabi_decodedOutput>(to_tonlib_api(values));
}

tonlib_api_ptr<tonlib_api::Object> TonlibClient::do_static_request(const tonlib_api::ftabi_unpackFromCell& request) {
  if (request.cell_ == nullptr) {
    return status_to_tonlib_api(TonlibError::EmptyField("cell"));
  }
  auto result = unpack_from_cell(request);
  if (result.is_error()) {
...
tonlib_api_ptr<tonlib_api::Object> TonlibClient::do_static_request(const tonlib_api::ftabi_unpackPublicKey& request) {
  ...
  return tonlib_api::make_object<tonlib_api::ftabi_unpackedPublicKey>(public_key_r.move_as_ok().key);
}
...
void run_local_cached(KeyStorage& key_storage, const tonlib_api::ftabi_runLocalCached& request,
  ...
  TRY_RESULT_PROMISE(promise, function, parse_function(*request.function_))
  TRY_RESULT_PROMISE(promise, function_call, parse_function_call(key_storage, *function, request.call_))
  TRY_RESULT_PROMISE(promise, root, vm::std_boc_deserialize(request.state_))

  TRY_RESULT_PROMISE(promise, result,
                     ftabi::run_smc_method(account_address, static_cast<ton::LogicalTime>(request.gen_lt_),
                                           static_cast<td::uint32>(request.gen_utime_), std::move(root),
                                           std::move(function), std::move(function_call)))

  promise.set_result(to_tonlib_api(result));
}

td::Status TonlibClient::do_request(const tonlib_api::ftabi_runLocalCached& request,
  ...
  run_local_cached(key_storage_, request, std::forward<std::decay_t<decltype(promise)>>(promise));
  return td::Status::OK();
}

void run_local_cached_split(KeyStorage& key_storage, const tonlib_api::ftabi_runLocalCachedSplit& request,
                            td::Promise<tonlib_api_ptr<tonlib_api::ftabi_tvmOutput>>&& promise) {
  TRY_RESULT_PROMISE(promise, account_address, get_account_address(request.address_->account_address_));
  TRY_RESULT_PROMISE(promise, function, parse_function(*request.function_))
  TRY_RESULT_PROMISE(promise, function_call, parse_function_call(key_storage, *function, request.call_))
  TRY_RESULT_PROMISE(promise, data, vm::std_boc_deserialize(request.data_))
  TRY_RESULT_PROMISE(promise, code, vm::std_boc_deserialize(request.code_))
  const auto balance = block::CurrencyCollection(request.balance_);
  TRY_RESULT_PROMISE(promise, message_body, function->encode_input(function_call))
...
td::Status TonlibClient::do_request(const tonlib_api::ftabi_runLocalCachedSplit& request,
                                    td::Promise<object_ptr<tonlib_api::ftabi_tvmOutput>>&& promise) {
  if (request.data_.empty()) {
    return TonlibError::EmptyField("data");
  }
  ...
  run_local_cached_split(key_storage_, request, std::forward<std::decay_t<decltype(promise)>>(promise));
  return td::Status::OK();
}
...
td::Status TonlibClient::do_request(const tonlib_api::exportKey& request,
  ...
  TRY_RESULT(parsed, from_tonlib_api(*request.input_key_));
  auto [type, input_key] = std::move(parsed);

  tonlib_api_ptr<tonlib_api::ExportedKey> result;

  switch (type) {
    case KeyStorage::InputKeyType::Fake:
    case KeyStorage::InputKeyType::Original: {
      TRY_RESULT(exported_key, key_storage_.export_key(std::move(input_key)));
      result = tonlib_api::make_object<tonlib_api::exportedKey>(std::move(exported_key.mnemonic_words));
      break;
    }
    case KeyStorage::InputKeyType::Ftabi: {
      TRY_RESULT(exported_key, key_storage_.export_ftabi_key(std::move(input_key)));
      result = tonlib_api::make_object<tonlib_api::ftabi_exportedKey>(std::move(exported_key.mnemonic_words),
...
td::Status TonlibClient::do_request(const tonlib_api::importKey& request,
  ...
  TRY_RESULT(key, downcast_call2<td::Result<KeyStorage::Key>>(
                      *request.exported_key_,
                      td::overloaded(
                          [&](tonlib_api::exportedKey& exported_key) {
                            return key_storage_.import_key(std::move(request.local_password_),
                                                           std::move(request.mnemonic_password_),
                                                           KeyStorage::ExportedKey{std::move(exported_key.word_list_)});
                          },
                          [&](tonlib_api::ftabi_exportedKey& exported_key) {
                            return key_storage_.import_ftabi_key(
                                std::move(request.local_password_),
                                KeyStorage::ExportedFtabiKey{std::move(exported_key.word_list_),
                                                             std::move(exported_key.derivation_path_)});
                          })));
...
td::Status TonlibClient::do_request(const tonlib_api::exportPemKey& request,
  ...
  TRY_RESULT(parsed, from_tonlib_api(*request.input_key_));
  auto [type, input_key] = std::move(parsed);
  if (type == KeyStorage::InputKeyType::Ftabi) {
    // TODO
    return TonlibError::Internal("Unimplemented");
  }
...
td::Status TonlibClient::do_request(const tonlib_api::exportEncryptedKey& request,
  ...
  TRY_RESULT(parsed, from_tonlib_api(*request.input_key_));
  auto [type, input_key] = std::move(parsed);
  TRY_RESULT(exported_key, key_storage_.export_encrypted_key(type, std::move(input_key), request.key_password_));
...
td::Status TonlibClient::do_request(const tonlib_api::exportUnencryptedKey& request,
  ...
  TRY_RESULT(parsed, from_tonlib_api(*request.input_key_));
  auto [type, input_key] = std::move(parsed);
  TRY_RESULT(exported_key, key_storage_.export_unencrypted_key(type, std::move(input_key)));
...
auto generate_extended_state_init(const tonlib_api::ftabi_generateExtendedStateInit& request)
-> td::Result<tonlib_api_ptr<tonlib_api::ftabi_stateInit>> {
  TRY_RESULT(tvc, vm::std_boc_deserialize(request.tvc_))
  TRY_RESULT(values, parse_state_init_values(request.values_));

  TRY_RESULT(state_init, ftabi::generate_state_init(tvc, values));
...
tonlib_api_ptr<tonlib_api::Object> TonlibClient::do_static_request(const tonlib_api::ftabi_generateExtendedStateInit& request) {
  auto result = generate_extended_state_init(request);
  if (result.is_error()) {
    return status_to_tonlib_api(result.move_as_error());
  }
  return result.move_as_ok();
...
template <class P>
td::Status TonlibClient::do_request(const tonlib_api::ftabi_generateExtendedStateInit& request, P&&) {
  UNREACHABLE();
  return TonlibError::Internal();
}
template <class P>
